...
class 技能类 extends 基类 {
  ...
  constructor(
    ...
    this.on('触发', async (参数: Record<string, unknown>) => {
      // 效果在触发的那一刻同步结算，不再等技能立绘面板播完：
      // 「攻击时」等时机必须先于攻击伤害结算，否则效果会落到攻击结算之后；
      // 立绘演出只负责表现，等它播完再结算会让伤害/迷雾延迟数秒才出现在棋盘上。
      // 同一技能实例可能在上一效果结算前连发（如同一威压雷霆给多体上雷印），
      // 每次触发都须基于当前棋盘重新选目标，避免复用已生效的旧目标导致白触发
      ...
      switch (this.效果描述) {
        ...
        case '替换此技能为完全离场的附属神技能':
          ...
          break
        case '获得效果值编号列表的随机一张神迹卡（不重复）':
          if (this.效果值.length) {
            this.效果值 = 随机类.乱序(this.效果值)
            const 编号 = this.效果值.pop() as number
            this.目标列表.forEach((v) => {
              if (v instanceof 玩家类) {
                v.emit('获得卡', { 编号 })
              }
    ...
    if (!仅查询目标) {
      ...
      case '效果值中合体素材单位同时在场时':
        ...
        break
      case '编号为选择范围的召唤物登场时':
        this.携带者.玩家.on('召唤物登场时', (参数: { 单位: 附属神类 }) => {
          if (参数.单位.编号 === this.选择范围) this.触发()
        })
        break
      case '主神生命值低于触发辅助时':
        this.携带者.玩家.on(
  ...
  消耗结算() {
    this.携带者.玩家.emit('行动点变化', { 变化值: -this.消耗 })
  }
      ...
      () => {
        ...
        行动队列类.发送通知({
          message: `触发技能：${this.技能名称}，携带者：${this.携带者.类型}${
            this.携带者.卡牌名称
          }：命${this.携带者.生命值}/攻${this.携带者.攻击力}/移${
            this.携带者.移动力
          } ${this.携带者.弹幕 ? '弹幕：' + this.携带者.弹幕.卡牌名称 : ''}`,
          caption: this.技能描述,
...
class 效果类 extends 基类 {
  ...
  constructor(描述: string, 发动者: 单位类, 目标: 目标类) {
    ...
    if (回合) {
      ...
      if (
        this.发动者.是否我方 != 玩家类.我方回合 &&
        this.目标.是否我方 != 玩家类.我方回合
      )
        this.持续回合++
    }
    if (
...
class 单位类 extends 目标类 {
  ...
  攻击可解除迷雾 = true
  // 本次攻击中因命中而解除迷雾的单位：由 攻击开雾() 在攻击动画开播前填好，
  // 供 攻击() 结算时派发「被命中的同时暴露时」（该事件须与伤害同时发生，故不提前）
  攻击命中并解除迷雾的单位列表: 单位类[] = []
  生命值 = 0
  生命上限 = 0
  护盾值 = 0
  ...
  constructor(玩家: 玩家类, 编号: number, 位置: 位置类) {
    ...
    this.on('秘术变化', (参数: { 变化值: 神迹卡类 | undefined }) => {
      ...
      if (this.秘术) {
        ...
        // 秘术一发动就撤掉头顶的 aomi 标记：不能等「触发时」，
        // 它要等秘术效果全部结算完才派发（效果里可能有选择、召唤、复活等流程），标记会滞留在小人头上
        this.秘术.技能.once('触发', () => {
          this.emit('秘术发动时')
        })
        this.秘术.技能.once('触发时', () => {
  ...
  // 开雾提前：攻击确认后、攻击动画开播前先解除攻击范围内的迷雾，
  // 让玩家在确认攻击的当下就看到棋盘开雾，而不是等攻击动画与受击特效播完。
  // 开雾及其引发的「攻击解除敌方单位迷雾时」仍属这次攻击的逻辑，只是提前执行；
  // 命中、伤害与「被命中的同时暴露时」仍留在 攻击() 里按原顺序结算。
  攻击开雾(攻击范围: 位置类[]) {
    const 攻击命中的单位列表: 单位类[] = []
    this.攻击命中并解除迷雾的单位列表 = []
    攻击范围.forEach((v) => {
      ...
      if (
        ...
        this.攻击命中并解除迷雾的单位列表.push(v.单位)
      }
    })
    if (this.攻击命中并解除迷雾的单位列表.length) {
  ...
  装填消耗结算(弹幕卡: 弹幕卡类) {
    this.玩家.emit('行动点变化', { 变化值: -弹幕卡.消耗 })
    弹幕卡.已使用 = true
  }
  装填弹幕(弹幕卡: 弹幕卡类) {
    // 「使用弹幕卡时」必须在弹幕真正挂到单位之后派发：监听它的「敌方装填弹幕卡时」
    // 类技能（如「海原的援军」「斯芬克斯」「净除」）会立刻对敌方全体【清除】，
    // 若此时 this.弹幕 还没赋值，清除就会漏掉这张刚装填的弹幕，导致清除失效
    this.弹幕 = 弹幕卡
    this.玩家.emit('使用弹幕卡时')
    // 弹幕若已被上述【清除】移除，视为装填未成功，不再触发弹幕自身效果
    const 装填成功 = this.弹幕 === 弹幕卡
    if (装填成功) {
...
class 神迹卡类 extends 牌类 {
  ...
  消耗结算() {
    this.玩家.emit('行动点变化', { 变化值: -this.消耗 })
