...
class 技能类 extends 基类 {
  ...
  static 技能效果描述: Record<string, string> = {
    ...
    '117': '攻击力+效果值%的编号为选择范围的单位的攻击力（向下取整）',
    ...
    '125': '攻击不可解除迷雾',
    '126': '抽效果值张弹幕卡',
    '127': '攻击力+按效果值列表中的概率随机判定一个值',
    '128': '攻击自身的单位完全离场',
    '129': '获得本局使用弹幕卡数量的效果值编号的卡',
    '130': '生命值+效果值%的自身护盾值（向下取整）',
    '131': '攻击力+效果值%的选择范围编号的单位的攻击力（向下取整）',
    '132': '随机召唤效果值列表里的一个单位',
    ...
    this.on('触发', async (参数: Record<string, unknown>) => {
      ...
      switch (this.效果描述) {
        ...
        case '召唤编号为效果值的单位':
          this.目标列表.forEach((v) => {
            if (v instanceof 位置类) {
              new 附属神类(this.携带者.玩家, this.效果值[0], v)
            }
          })
          break
        ...
        case '生命值+效果值乘以敌方单位的数量':
          ...
          break
        case '攻击力+效果值%的编号为选择范围的单位的攻击力（向下取整）':
          {
        ...
        case '生命值+效果值%的自身护盾值（向下取整）':
          ...
          break
        case '攻击力+效果值%的选择范围编号的单位的攻击力（向下取整）':
          {
            const 攻击力变化值 = Math.floor(
              (this.携带者
                .我方(单位类)
...
class 单位类 extends 目标类 {
  ...
  可装填(弹幕卡: 弹幕卡类) {
    return this.可否装填 && 玩家类.行动点 >= 弹幕卡.消耗
  }
  装填消耗结算(弹幕卡: 弹幕卡类) {
...
function 重置() {
  目标类.目标列表 = []
  效果类.效果列表 = []
  玩家类.事件 = new EventEmitter()
  玩家类.游戏已开始 = false
  玩家类.我方回合 = undefined
  玩家类.行动点 = 10
  玩家类.倒计时 = Date.now() + 1000 * 60
}
export {
  主神类,
  位置类,
