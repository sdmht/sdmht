...
/** 临时诊断：给每次效果入闸分配序号，用于核对「入闸-放行」是否一一对应（定位后删除） */
let 演出调试序号 = 0
...
class 技能类 extends 基类 {
  ...
  constructor(
    ...
    this.on('触发', async (参数: Record<string, unknown>) => {
      ...
      const 演出序号 = !this.来源卡片 ? ++演出调试序号 : 0
      if (演出序号) {
        console.warn(
          `[演出调试]入闸 #${演出序号} 技能${this.编号}:${this.技能名称} 携带者:${this.携带者.卡牌名称} 回合次数:${this.本回合使用次数} 展示中:${行动队列类.行动队列.技能展示中} 等待者:${行动队列类.行动队列.技能展示完成等待者.length}`
        )
      }
      await 等技能演出播完(!this.来源卡片)
      if (演出序号) {
        console.warn(
          `[演出调试]放行 #${演出序号} 技能${this.编号}:${this.技能名称} 剩余等待者:${行动队列类.行动队列.技能展示完成等待者.length}`
        )
      }
      // 同一技能实例可能在上一效果结算前连发（如同一威压雷霆给多体上雷印），
      // 入闸时清过的缓存已被前一触发结算时写入，这里须在结算前重新置空，
      // 让每次触发都基于当前棋盘重新选目标，避免复用已生效的旧目标导致白触发
      this._目标列表缓存 = undefined
      switch (this.目标类型) {
        case '攻击命中的单位':
