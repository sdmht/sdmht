...
async function 加载动画(场景: Scene, key: string, spine路径: string, atlas路径: string) {
...
function 加载普攻动画(场景: Scene, 美术资源: number[]) {
  const spine文件 = 获得资源(美术资源, (f, i) => f === `spine/${i}/effect-${i}.json`)
  const atlas文件 = 获得资源(美术资源, (f, i) => f === `spine/${i}/effect-${i}.atlas`)
  if (spine文件 && atlas文件) {
    return 加载动画(场景, 美术资源.toString(), spine文件, atlas文件)
  }
}
