...
onMounted(async () => {
  ...
  行动队列类.行动队列.on(
    ...
    async (是否我方: boolean, ...行动: 行动类型) => {
      try {
        ...
        if (行动[0] == '回合结束') {
          const 即将开始回合的玩家 = 是否我方 ? 玩家.敌方玩家 : 玩家
          // 同一个回合可能被双端各结束一次（双方倒计时同时到点，或一方超时救援）：
          // 若该玩家的回合已经开始，说明这条「回合结束」是重复的，跳过。
          // 否则会重复开回合，让回合数、迷雾恢复倒计时等按次数推进的状态在双端错位。
          if (玩家类.我方回合 !== 即将开始回合的玩家.是否我方) {
            即将开始回合的玩家.回合开始()
          }
        }
        if (行动[0] == '神威') {
        ...
        if (行动[0] == '装填弹幕') {
          ...
          单位.攻击前()
          // 确认攻击后立即开雾：不等攻击动画与受击特效播完，
          // 玩家点下攻击的当帧就解除攻击范围内的迷雾
          单位.攻击开雾(攻击范围)
  ...
  // 对手回合超时且对手始终没有任何行动（卡死/掉线）时，由本机代为结束对手回合。
  // 必须按「对手发起」注入：若走 添加()（= 我方发起），双端都会把它当成「对手的回合结束」，
  // 反而把回合重新开给对手，回合永远切不回来。
  function 强制结束对手回合() {
    if (玩家类.我方回合 || 有结算动画进行中()) return
    选中的单位.value = undefined
    行动队列类.行动队列.注入对手行动(['回合结束'])
  }
  ...
  let 上一帧时间 = Date.now()
  // 对手回合超时的救援计时（对手卡死/掉线时本机代为结束回合，见定时器内注释）
  let 对手回合超时起点: number | undefined
  // 对手倒计时到点后先等这么久，等对手自己的「回合结束」送达；仍未等到才由本机代为结束
  const 对手回合超时宽限 = 3000
  强制结束回合 = setInterval(() => {
    ...
    if (倒计时 > 0) {
      // 倒计时被重置（说明有行动刚结算，双方都还活着），取消对手回合的救援计时
      对手回合超时起点 = undefined
      if (倒计时 < 3) {
        倒计时潜行.visible = false
        倒计时结束.visible = true
      ...
      if (玩家类.我方回合) {
        ...
        // 对手回合超时：对手卡死/掉线时他永远不会发来「回合结束」，必须由本机代为结束，
        // 否则这里只是反复重置倒计时，回合永远切不回来。
        // 但对手只是稍慢时不能抢跑：先给一段宽限期等他自己结束回合，
        // 他的「回合结束」一到，结算会重置倒计时，下个 tick 就把救援计时清掉。
        if (对手回合超时起点 === undefined) {
          对手回合超时起点 = 现在
        } else if (现在 - 对手回合超时起点 >= 对手回合超时宽限) {
          对手回合超时起点 = undefined
          强制结束对手回合()
        }
      }
