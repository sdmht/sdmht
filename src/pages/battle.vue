...
onMounted(async () => {
  ...
  async function 显示技能UI(技能信息: {
    ...
    入队并折叠同角色连续(技能信息)
    console.warn(
      `[演出调试]面板入队 ${技能信息.技能名称} 载者:${技能信息.携带者编号} 队内:${技能展示队列.length} 展示中:${正在展示技能} 等待者:${行动队列类.行动队列.技能展示完成等待者.length} 离场:${技能信息.是否离场演出}`
    )
    if (正在展示技能) return
    正在展示技能 = true
    ...
    while (技能展示队列.length > 0) {
      ...
      try {
        ...
        console.warn(
          `[演出调试]面板播完 ${当前技能.技能名称} 离场:${当前技能.是否离场演出} 播完前等待者:${行动队列类.行动队列.技能展示完成等待者.length}`
        )
        行动队列类.行动队列.通知技能面板播完()
      } catch (e) {
        // 出错时清空展示层并重置透明度
    ...
    console.warn(
      `[演出调试]周期结束 复位展示中:${行动队列类.行动队列.技能展示中} 兜底放行等待者:${行动队列类.行动队列.技能展示完成等待者.length}`
    )
    行动队列类.行动队列.通知技能展示完成()
  }
