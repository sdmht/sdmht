<template>
  <q-list>
    ...
    <template v-for="(卡组, k) in 编号卡组列表" :key="k">
      <q-slide-item
        :style="`width: ${$q.screen.width > $q.screen.height ? 5 : 10}0%; display: inline-block`"
        left-color="dark"
        right-color="red"
        @left="
          (e) => {
        ...
        <q-item clickable v-ripple @click="复刻卡组(卡组)">
          <div>
            ...
            <img
              ...
              :src="`maincard/CharacterHeadM_${获得主神信息(卡组.主神).美术资源}.webp`"
            />
            <img
              ...
              :src="`character/CharacterHeadL_${获得附属神信息(编号).美术资源}.webp`"
            />
          </div>
        </q-item>
      </q-slide-item>
              ...
              (val, update) => {
                update(() => {
                  ...
                  可选附属神列表 = 可选附属神列表.sort(
                    (a, b) => parseInt(a.品质) - parseInt(b.品质),
                  )
                })
              }
            "
...
import type { 编号卡组类型 } from 'src/utils/卡组'
import { 字符串转编号卡组, 编号卡组转字符串 } from 'src/utils/卡组'
...
const 主神列表 = 主神信息列表
  .filter((x) => parseInt(x.仅系统用) == 0)
  .reduce(
    (pv, v) => {
      const 主神信息 = 获得主神信息(parseInt(v.编号))
      const 技能信息 = [
        获得技能信息(parseInt(主神信息.技能1)),
        获得技能信息(parseInt(主神信息.技能2)),
        获得技能信息(parseInt(主神信息.技能3)),
      ]
      技能信息.forEach((x, i) => {
        const 附带技能 =
          typeof x.附带技能 === 'string'
            ? x.附带技能.split(',').map((x) => parseInt(x))
            : [x.附带技能]
        let 被动技能: ReturnType<typeof 获得技能信息> | undefined = undefined
        for (let i = 附带技能.length - 1; i >= 0; i--) {
          const 技能 = 附带技能[i]
          if (技能 && 技能 != 1) {
            const 信息 = 获得技能信息(技能)
            if (信息.何时触发) {
        ...
        pv.push({
          主神: parseInt(v.编号),
          卡牌名称: v.卡牌名称,
          美术资源: parseInt(v.美术资源),
          主神技能: (i + 1) as 1 | 2 | 3,
          技能名称: x.技能名称,
          技能描述: x.技能描述,
          技能消耗: parseInt(x.消耗),
          回合最大使用次数: parseInt(x.回合最大使用次数),
          单场最大使用次数: parseInt(x.单场最大使用次数),
          被动技能名称: 被动技能?.技能名称 ?? '',
          被动技能描述: 被动技能?.技能描述 ?? '',
        })
      })
...
function 更新可选神迹卡列表(_神迹卡列表: typeof 神迹卡列表) {
  可选神迹卡列表.value = _神迹卡列表
    .concat(
      _神迹卡列表
        .filter((x) => parseInt(x.品质) > 1)
        .map((x) => ({
          ...x,
          卡牌名称: x.卡牌名称 + ' 2',
        })),
    )
    .sort((a, b) => parseInt(a.排序编号) - parseInt(b.排序编号))
    .sort((a, b) => parseInt(a.品质) - parseInt(b.品质))
}
更新可选神迹卡列表(神迹卡列表)
  ...
  get 卡组() {
    if (
      ...
      return {
        ...
        附属神: this.附属神.value,
        神迹卡: this.神迹卡.value.map((x) => parseInt(x.编号)),
        弹幕卡: this.弹幕卡.value.map((x) => parseInt(x.编号)),
      }
    }
  }
  重置() {
...
function 复刻卡组(_卡组: 编号卡组类型) {
  ...
  _卡组.神迹卡.forEach((x) => {
    const 神迹卡 = 可选神迹卡列表.value.find(
      (y) => parseInt(y.编号) == x && !卡组.神迹卡.value.includes(y),
    )
    if (神迹卡) {
  ...
  _卡组.弹幕卡.forEach((x) => {
    const 弹幕卡 = 可选弹幕卡列表.value.find(
      (y) => parseInt(y.编号) == x && !卡组.弹幕卡.value.includes(y),
    )
    if (弹幕卡) {
