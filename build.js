...
(async () => {
  ...
  await (
    await page.waitForSelector("#welcomeTourDialog .noThanksLink")
  ).click();
  ...
  for (const file of await readdirp(".", {
    type: "files",
    directoryFilter: (f) =>
      ![".git", "node_modules", "ts-defs", "dist"].includes(f.basename),
    fileFilter: (f) =>
      ![/^\./, /\.uistate\.json$/].some((v) => v.test(f.basename)) &&
      !["build.js", "package.json", "yarn.lock", "tsconfig.json"].includes(
        f.basename
      ),
  })) {
